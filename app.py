        ...
        with st.status("Initializing Ghost Coder...", expanded=True) as status:
            ...
            for event in graph.stream(initial_state):
                for node_name, node_state in event.items():
                    st.write(f"✅ **{node_name.capitalize()} Agent** finished executing.")
            ...
            with tab3:
                if final_state.get("test_passed"):
                    ...
                    if st.button("Submit Pull Request"):
                        with st.spinner("Creating Pull Request on GitHub..."):
                            # This calls the github integration tool. Note: In reality, you would need to run `git push` on the workspace first.
